...
const ContinueWatchingRow = ({ filterType = 'all' }) => {
    ...
    historyList.forEach(item => {
        ...
        if (!seenIds.has(uniqueKey) && matchesFilter) { seenIds.add(uniqueKey); uniqueHistory.push(item); } 
    });
        ...
        <div className="mb-6 mt-4">
            ...
            <div className="flex overflow-x-auto gap-3 px-4 pb-2 hide-scrollbar snap-x">
                {uniqueHistory.slice(0, 15).map(item => {
                    ...
                    return (
                    <div key={item.historyId} onClick={() => openDetails(item)} className={`flex-shrink-0 cursor-pointer snap-start relative group focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:z-10 rounded-md ${isLive ? 'w-32' : 'w-40'}`}>
                        <div className={`relative w-full ${isLive ? 'aspect-[4/3] bg-neutral-900 p-2' : 'aspect-video bg-neutral-800'} rounded-md overflow-hidden border border-neutral-700 flex flex-col items-center justify-center`}>
                            ...
                            {!isLive && (
                                <div className="absolute bottom-0 left-0 right-0 h-1 bg-neutral-700 z-10">
                                    <div className="h-full bg-green-500 shadow-[0_-1px_6px_rgba(34,197,94,0.6)]" style={{ width: `${item.progress || 10}%` }}></div>
                                </div>
                            )}
